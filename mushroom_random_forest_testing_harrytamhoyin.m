...
% Load the training script for the test data and the best trained random
% forest model
load('mushroom_random_forest_training_harrytamhoyin.mat')

% Apply the trained random forest model on test set for prediction and count
% the test time
% Note if there is error for X_test, please run the training script first
test_start_time = tic;
predictions_test = str2double(predict(best_rf, X_test));
test_time = toc(test_start_time);

% Calculate AUC value for the test set
[X_val_test, Y_val_test, T_val_test, AUC_val_test] = perfcurve(Y_test, predictions_test, '14');
...
title('ROC Curve For Random Forest (Test Set)');
legend('show');
