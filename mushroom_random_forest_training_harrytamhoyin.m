...
% Perform 10 fold cross valiadation with hyperparameters
for fold = 1:10
    ...
    % Train the random forest model on the current training fold
    % Number of trees: 15.
    % Number of features to select in each split: 9 
    rf = TreeBagger(15, X_train_fold, Y_train_fold, 'Method', 'classification', ...
        'OOBPrediction', 'on', 'Options', statset('UseParallel', true), ...
    ...
    predictions_fold = str2double(predict(rf, X_validation_fold));
    ...
    [X_val_fold, Y_val_fold, T_val_fold, AUC_val_fold] = perfcurve(Y_validation_fold, predictions_fold, '14');
    ...
    % AUC value
    if AUC_val_fold > best_rf_auc
        best_rf_auc = AUC_val_fold;
        best_rf = rf;
    end
end
...
title('ROC Curve For Random Forest (10 Fold Cross Validation)');
legend('show');
...
title('Confusion Matrix For Random Forest (10 Fold Cross Validation)');

% Display the best model with the highest AUC value
disp(['Best AUC value: ' num2str(best_rf_auc)]);

% Save the best model
save('mushroom_random_forest_training_harrytamhoyin.mat', 'best_rf')